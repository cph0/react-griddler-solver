import { sort, discardB, discardL, assert, hasB, hasL } from './util';

// pack two 16bit ints into one so we can emulate tuple pairs
var MAXPAIRVAL = 0xFFFF;
export function hWord(x) { return (x >>> 16) & 0xFFFF; };
export function lWord(x) { return x & 0xFFFF; };
export function makePair(x, y) { return (x << 16) | y; };

// A queue that ensures uniqueness of elements via an instrusive flag.
// Uses x.inqueue to store queue membership
export function IntrusiveQueue(vals) {
    this.q = [];
    vals = vals || [];
...
export function CellNode(ind, debuginfo) {
    this.ind = ind;
    this.vals = [];
...
export function GapNode(ind, iscol, roworcolnum, debuginfo) {
    this.ind = ind;
    this.iscol = iscol;
...
export function RevCellNode(iscol, cpair, choices) {
    this.iscol = iscol;
    this.cpair = cpair;
...
export function Puzzle(R, C, cells, gaps, callback) {
